...
NOTE 2
the xhr.onload is for what we want to do with the information we get from the website. The idea was to display the jokes on the website, so i created a variable for the ul on the html page. the jokes are stored in array form in joke.value (jokeArr). Because the respons eis an objevct, so we need to get its value. APIs are formatted differently, some will return an arrayt so it's easy to loop through. this is what I will loop over using a for-loop. but first make sure that the type is success. i created a list element, and a text node with the specific joke. I then appended the textnode onto the list item, and the list item onto the ul
